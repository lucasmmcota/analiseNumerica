...
##    VetX -> Abscissas
##    VetY -> Solução do PVI
##    DifY -> Diferenças entre as aproximações y(i+1) de ordens 5 e 4
##
## @seealso{}
## @end deftypefn
...
function [VetX, VetY, DifY] = dormand_prince (a, b, y0, m)
  ...
  for i = 1 : m
    ...
    xt = a + i * h;
    % aproximação de ordem 4: coeficientes bcj(=bj)
    y4t = yt + h * (bc1 * k1 + bc3 * k3 + bc4 * k4 + bc5 * k5 + bc6 * k6 + bc7 * k7);
    % aproximação de ordem 5: coeficientes bj = a7j
